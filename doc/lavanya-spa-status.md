...
|---|---|---|
| `f12bb3e` | **Bug fix**: Helpdesk-native `Open`/`Replied` tickets were invisible on Today's Work (classifier didn't map them) — the "new ticket not showing after refresh" report. Active tickets matching no bucket now surface as new complaints. | live data (real Open ticket 0001) + regression test TW-014b; 23 today_work tests pass |
