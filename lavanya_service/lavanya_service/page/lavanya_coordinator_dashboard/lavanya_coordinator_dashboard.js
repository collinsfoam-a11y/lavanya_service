/**
 * Lavanya Coordinator Dashboard
 *
 * Uses the Stitch design system to present Coordinator-level summary metrics
 * and drill-downs into pending queues.
 */

frappe.pages["lavanya-coordinator-dashboard"].on_page_load = function (wrapper) {
	const page = frappe.ui.make_app_page({
		parent: wrapper,
		title: __("Lavanya Coordinator Dashboard"),
		single_column: true,
	});
	...
	const GROUP_ACCENT = {
		triage_queue: "#ba1a1a",
		unassigned_tickets: "#5412dd",
		pending_parts: "#a84900",
	};
	...
	function styleBlock() {
		...
		.lv-tw .lv-metric.lv-accent-red { border: 2px solid #ba1a1a; background: #fff1f0; }
	...
	function load() {
		...
		frappe.call({
			method: "lavanya_service.api.coordinator_dashboard.get_coordinator_dashboard",
			args: { include_counts: 1 },
			callback: (r) => render(r.message || {}),
			error: () => {
				$body.html(
					`<div class="lv-tw"><div style="padding: 32px; color: #ba1a1a;">${__(
						"Could not load Coordinator Dashboard."
					)}</div></div>`
				);
			},
		});
	...
	function renderSummary(summary, date) {
		const metric = (label, value, accentClass) =>
			`<div class="lv-metric${accentClass ? " " + accentClass : ""}">
				<div class="lv-num" style="${accentClass === 'lv-accent' ? "color:#5412dd;" : accentClass === 'lv-accent-red' ? "color:#ba1a1a;" : ""}">${esc(value || 0)}</div>
				<div class="lv-cap">${esc(label)}</div>
			</div>`;

		return `<div class="lv-summary">
			${metric(__("Triage Queue"), summary.triage, "lv-accent-red")}
			${metric(__("Unassigned Tickets"), summary.unassigned, "lv-accent")}
			${metric(__("Pending Parts"), summary.parts, "")}
	...
	function renderRow(ticket) {
		const status = ticket.status || ticket.ticket_status || "New";
		...
		const reason = ticket.pending_reason || ticket.closure_type || ticket.current_custody_status || "";
